  ...
  static Future<List<Map<String, String>>> search(String query, {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        List<Map<String, String>> results = features.map<Map<String, String>>((feature) {
          ...
          if (props['housenumber'] != null && props['housenumber'].toString().isNotEmpty) {
            streetPart = '${props['name'] ?? props['street'] ?? ''} ${props['housenumber']}'.trim();
          } else {
            streetPart = (props['name'] ?? props['street'] ?? '').toString().trim();
          }
        ...
        if (results.isEmpty && RegExp(r'\d').hasMatch(query)) {
          ...
          String queryWithoutNumber = query.replaceAll(RegExp(r'\d+'), '').replaceAll(RegExp(r',\s*,'), ',').trim();
